...
## Path handling
- Do not assume Windows paths (C:\\, D:\\) work inside Bash. Use \`powershell\` for Windows paths.
- Do not manually convert Windows paths to \`/mnt/c/...\` unless the active Bash environment has been verified to support it.
- Do not assume WSL is available.

## Long-running commands
- Do not use Bash background syntax (\`command &\`). Use \`win_start_process\` instead.
- For stuck ports, use \`win_kill_port\`.`;
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      "Do NOT use powershell for git or Unix-style text processing — use bash for those.",
    ],
